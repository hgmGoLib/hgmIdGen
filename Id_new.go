...
func encodeVarLenUint32(dst []byte, val uint32) int {
	val &= 0x3FFFFFFF // 只保留低30位，清除高2位
	switch {
	case val <= 0x3F: // 6 bits
		dst[0] = byte(val & 0x3F) | 0x00 // 前缀00
		return 1
	case val <= 0x3FFF: // 14 bits
		dst[0] = byte((val>>8)&0x3F) | 0x40 // 前缀01
		dst[1] = byte(val)
		return 2
	case val <= 0x3FFFFF: // 22 bits
		dst[0] = byte((val>>16)&0x3F) | 0x80 // 前缀10
		dst[1] = byte(val >> 8)
		dst[2] = byte(val)
		return 3
	default: // up to 30 bits
		dst[0] = byte((val>>24)&0x3F) | 0xC0 // 前缀11
		dst[1] = byte(val >> 16)
		dst[2] = byte(val >> 8)
		dst[3] = byte(val)
		return 4
